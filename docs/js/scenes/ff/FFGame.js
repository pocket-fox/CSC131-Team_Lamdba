...
var FFGameState = {
  ...
  create: function () {
    ...
    for (var i = 0; i < FFGame.options.length; ++i) {
      ...
      var outlineSprite = this.add.sprite(
        spriteData.position.x * WIDTH,
        spriteData.position.y * HEIGHT,
        spriteData.name + "_outline"
      );
      ...
      for (var j = 0; j < spriteData.extras.length; ++j) {
        ...
        var extraSprite = this.add.sprite(
          extra.position.x * WIDTH,
          extra.position.y * HEIGHT,
          extra.name
        );
      ...
      if (option.done) {
        ...
        if (option.wrong) {
          ...
          optionSprite.clickable.position.setTo(
            correctSpriteData.position.x * WIDTH,
            correctSpriteData.position.y * HEIGHT
          );
          optionSprite.clickable.scale.setTo(
            correctSpriteData.scale.x,
            correctSpriteData.scale.y
          );
          for (var k = 0; k < optionSprite.extras.length; ++k) {
            optionSprite.extras[k].loadTexture(
              correctSpriteData.extras[k].name
            );
            optionSprite.extras[k].position.setTo(
              correctSpriteData.extras[k].position.x * WIDTH,
              correctSpriteData.extras[k].position.y * HEIGHT
            );
            optionSprite.extras[k].scale.setTo(
              correctSpriteData.extras[k].scale.x,
              correctSpriteData.extras[k].scale.y
            );
          }
        }
      }
    ...
    // Sprinklers
    this.sprinklerSprite1 = this.add.sprite(
      0.46 * WIDTH,
      0.58 * HEIGHT,
      "ff_sprinkler"
    );
    ...
    this.sprinklerSprite2 = this.add.sprite(
      0.58 * WIDTH,
      0.44 * HEIGHT,
      "ff_sprinkler"
    );
    ...
    this.questionBoxSprite = this.add.sprite(
      0.0 * WIDTH,
      0.0 * HEIGHT,
      "ff_question_box"
    );
    ...
    this.questionHeaderText = this.add.text(
      0.0 * WIDTH,
      -0.17 * HEIGHT,
      "LINE 1\nLINE 2",
    ...
    this.resultsHeaderText = this.add.text(
      0.0 * WIDTH,
      -0.43 * HEIGHT,
      "",
      TextStyle.centeredXXLarge
    ...
    this.resultsUpperText = this.add.text(
      0.0 * WIDTH,
      -0.34 * HEIGHT,
      "",
      TextStyle.centered
    ...
    this.resultsLowerText = this.add.text(
      0.0 * WIDTH,
      0.175 * HEIGHT,
      "",
  ...
  startResult: function (fixIt) {
    ...
    this.resultsHeaderText.setText(
      FFGameData.resultsHeader[!correct ? 2 : !fixIt ? 1 : 0]
    );
    ...
    if (correct) {
      ...
      if (option.wrong) {
        ...
        sprite.clickable.position.setTo(
          spriteData.position.x * WIDTH,
          spriteData.position.y * HEIGHT
        );
        ...
        for (var i = 0; i < sprite.extras.length; ++i) {
          ...
          sprite.extras[i].position.setTo(
            spriteData.extras[i].position.x * WIDTH,
            spriteData.extras[i].position.y * HEIGHT
          );
          sprite.extras[i].scale.setTo(
            spriteData.extras[i].scale.x,
            spriteData.extras[i].scale.y
          );
        }
      }
    } else {
